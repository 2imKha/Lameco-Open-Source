...
def run_logo_detector(file_path, kind):
    ...
    """
    from yolo.integration_bundle.inference import predict
    from PIL import Image

    def _grid_cell(cx, cy, width, height, cols, rows):
    ...
    def _grid_bounds(width, height, cols, rows, col_min, col_max, row_min, row_max):
        if width <= 0 or height <= 0 or cols <= 0 or rows <= 0:
            return None
        cell_w = width / float(cols)
        cell_h = height / float(rows)
        min_x = (col_min - 1) * cell_w
        max_x = col_max * cell_w
        min_y = (row_min - 1) * cell_h
        max_y = row_max * cell_h
        return (min_x, min_y, max_x, max_y)

    def _intersection_ratio(box, bounds):
        try:
    ...
    def _target_desc(kind):
        if kind == "pptx":
            return "W24, W25, X24, X25, Y24, Y25"
        return "W14, W15, X14, X15"

    def _is_in_logo_cell(box, width, height, kind):
        ...
        if kind == "pptx":
            cols, rows = 26, 26
            col_min, col_max = 23, 25  # W..Y
            row_min, row_max = 24, 25
        else:
            cols, rows = 25, 16
            col_min, col_max = 23, 24  # W..X
            row_min, row_max = 14, 15

        bounds = _grid_bounds(width, height, cols, rows, col_min, col_max, row_min, row_max)
        if not bounds:
            return False
        min_x, min_y, max_x, max_y = bounds

        # Allow a small tolerance inside the target grid (to reduce false negatives),
        # but still reject logos that touch the slide/page edges.
        tol_x = width * 0.02
        tol_y = height * 0.02
        safe_margin_x = width * 0.01
        safe_margin_y = height * 0.01
        min_x -= tol_x
        max_x += tol_x
        min_y -= tol_y
        max_y += tol_y

        # Require the full bbox to fit inside the allowed cells (no stretching outside).
        inside = x1 >= min_x and x2 <= max_x and y1 >= min_y and y2 <= max_y

        # Reject if the logo touches the slide/page edges.
        if x1 <= safe_margin_x or y1 <= safe_margin_y or x2 >= (width - safe_margin_x) or y2 >= (height - safe_margin_y):
            return False

        # Also keep the center check for a bit of tolerance at the edges.
        col, row = _grid_cell(cx, cy, width, height, cols, rows)
        if col is None or row is None:
            return False
        center_ok = col_min <= col <= col_max and row_min <= row <= row_max
        if not center_ok:
            return False

        # If bbox isn't fully inside, allow it if most of it overlaps the target cells.
        if inside:
            return True
        overlap = _intersection_ratio([x1, y1, x2, y2], (min_x, min_y, max_x, max_y))
        return overlap >= 0.65
    ...
    tmp_dir = tempfile.mkdtemp(prefix="logo_detect_")
    try:
        try:
            if kind == "pptx":
                thumbs = render_ppt_thumbnails(file_path, tmp_dir)
            elif kind == "docx":
                thumbs = render_docx_thumbnails(file_path, tmp_dir)
            else:
                return summary
        except Exception:
            return summary

        for item in thumbs or []:
            page = item.get("page")
            ...
            dets = predict(img_bytes, conf=0.25, iou=0.45)
            if not dets:
                summary["issues"].append({
                    "page": page,
                    "label": "logo_missing",
                    "message": f"Logo missing from the required grid cells ({_target_desc(kind)}).",
                })
                continue

            summary["pages_with_logo"] += 1
            in_br = False
            best = None
            best_score = -1.0
            for d in dets:
                ...
                if score > best_score:
                    best_score = score
                    best = d
                if _is_in_logo_cell(d.get("box", []), width, height, kind):
                    in_br = True
            ...
            if in_br:
                ...
                issue = {
                    "page": page,
                    "label": "logo_wrong_placement",
                    "message": f"Logo detected but not in the required grid cells ({_target_desc(kind)}).",
                }
                if best and best.get("box"):
                    issue["bbox"] = best["box"]
                    issue["score"] = best.get("score")
  ...
  <div class="shell">
    ...
    {% if job %}
      ...
      <p class="muted" style="margin-top:.75rem;">
        Logo placement checks run automatically after upload. See the "Logo Placement" section for results.
      </p>
    {% endif %}
  </div>
...
def upload():
    ...
    try:
        _logo_summary = run_logo_detector(src_path, kind) or {}
        if isinstance(_logo_summary, dict):
            _logo_issues = _logo_summary.get("issues", []) or []
        else:
            _logo_issues = _logo_summary or []
            _logo_summary = {
                "issues": _logo_issues,
                "pages_checked": 0,
                "pages_with_logo": 0,
  ...
  <div class="wrap">
    <div class="grid">
      ...
      <div>
        ...
        <div class="card">
          <div class="hd"><h3>Logo Placement</h3></div>
          <div class="bd">
            {% if job %}
              {% if job.logo_summary and job.logo_summary.pages_checked %}
                <div class="muted" style="font-size:12px;">
                  Checked {{ job.logo_summary.pages_checked }} page(s).
                </div>
                {% if job.logo_issues %}
                  <table>
                    <thead>
                ...
                {% else %}
                  <div style="color:#16A34A; font-weight:600;">
                    Logo placement OK on all pages (grid target).
                  </div>
                {% endif %}
              {% else %}
                <div class="muted" style="font-size:13px;">
                  Logo check skipped (no rendered pages available).
                </div>
              {% endif %}
            {% else %}
              <div class="muted" style="font-size:13px;">
                Upload a document to run the logo check.
              </div>
            {% endif %}
          </div>
        </div>
